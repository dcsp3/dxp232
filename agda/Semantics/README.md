# Semantics Guide

These are the semantics of our framework.

## Hierarchy

- Foundations
  - [Variance.lagda.md](Variance.lagda.md)
- Refinement Rules
  - [SchemaRefinement.lagda.md](SchemaRefinement.lagda.md)
  - [EndpointRefinement.lagda.md](EndpointRefinement.lagda.md)
  - [APIRefinement.lagda.md](APIRefinement.lagda.md)
- Refinement Proofs
  - [SchemaRefinementProperties.lagda.md](SchemaRefinementProperties.lagda.md)
  - [EndpointRefinementProperties.lagda.md](EndpointRefinementProperties.lagda.md)
  - [APIRefinementProperties.lagda.md](APIRefinementProperties.lagda.md)
- Main Results
  - [DecidableRefinement.lagda.md](DecidableRefinement.lagda.md)
  - [Drift.lagda.md](Drift.lagda.md)
  - [DriftProperties.lagda.md](DriftProperties.lagda.md)
  - [Compatibility.lagda.md](Compatibility.lagda.md)
